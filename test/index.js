  ...
  'Global': {
    'new': {
      '()': function() {
        ...
        assert.equal ( storage2.url, 'global:///default-test' );
      ...
      '("url")': function() {
        ...
        var storage2 = new Storage('global:///custom');

        assert.equal ( storage2.url, 'global:///custom' );
        assert.typeOf ( storage2.options, 'object' );
        assert.deepEqual ( storage2.options.custom, undefined );
      },
      ...
      '("url", options)': function() {
        ...
        var storage2 = new Storage('global:///custom', {custom: {foo: 'bar'}});
    ...
    '.defaults': function() {
      ...
      assert.equal ( Storage.defaults.url, 'global:///default-test' );
      assert.typeOf ( Storage.defaults.options, 'object' );
    },

    '.url': function() {
      assert.typeOf ( Storage.url, 'string' );
      assert.equal ( Storage.url, 'global:///default-test' );
    },
    ...
    '.reset()': function() {
      ...
      Storage.url = "global:///custom";
      assert.equal ( Storage.url, "global:///custom" );
